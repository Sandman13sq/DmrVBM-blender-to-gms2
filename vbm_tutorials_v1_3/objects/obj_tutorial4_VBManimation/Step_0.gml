...
{
	vbm_treat.Animator().PauseToggle();
}

// Navigate Animations
if (keyboard_check_pressed(ord("Z"))) 
{
	playbackkeyindex = (playbackkeyindex+1) mod vbm_treat.AnimationCount();
	vbm_treat.Animator().PlayAnimation(vbm_treat.AnimationGet(playbackkeyindex));
}
...
if (keyboard_check_pressed(ord("X"))) 
{
	...
	{
		vbm_treat.OptimizeAnimations();
	}
	else
	{
		vbm_treat.Animator().forcelocalposes ^= 1;
	}
}
...
// Update Animator
vbm_treat.Animator().Update(playbackspeed);
