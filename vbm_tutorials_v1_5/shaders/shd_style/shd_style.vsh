...
attribute vec4 in_Colour0;         // Normal = (nx, ny, nz, 0)
attribute vec4 in_Colour1;         // Color = (r,g,b,a)
...
void main()
{
    ...
    vec4 normal = vec4(in_Colour0.xyz * vec3(2.0)-vec3(1.0), 0.0);	// Normalize to [-1.0:1.0] range
	ivec4 bone = ivec4(in_Colour2 * 255.0);	// Convert to integer vector
	vec4 weight = vec4(in_Colour3);
	
	// Vertex Skinning
	mat4 mskin = mat4( float(u_bonematrices[0][3][3]==0.0) );	// Identity matrix if uniform is not set
	for (int i = 0; i < 4; i++) {
		mskin += u_bonematrices[bone[i]] * weight[i];
	}
