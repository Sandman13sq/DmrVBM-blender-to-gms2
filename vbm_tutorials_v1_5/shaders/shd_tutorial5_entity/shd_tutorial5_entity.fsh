...
uniform mat4 u_axes;	// <right, up, forward, p>

void main()
{
	// Params -----------------------------------------------------
	vec3 normal = normalize(v_vNormal);
	vec3 incoming = normalize(-u_axes[2].xyz);
	vec3 lightvec = -normalize(u_axes[2].xyz-vec3(0,0,40));
	...
	float shadowvalue = dp+v_vColour[1]*2.0-1.0;
	float shinevalue = dr+v_vColour[2]*2.0-1.0;
