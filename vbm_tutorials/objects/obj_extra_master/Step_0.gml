...
if (keyboard_check_pressed(ord("L"))) {lightmode ^= 1;}

// View-Dependent Lighting
if (lightmode)
{
	var d = 100;
	lightpos = [-viewforward[0], -viewforward[1], -viewforward[2] + 1];
	lightpos = [
		d * lightpos[0] / point_distance_3d(0,0,0, lightpos[0], lightpos[1], lightpos[2]),
		d * lightpos[1] / point_distance_3d(0,0,0, lightpos[0], lightpos[1], lightpos[2]),
		d * lightpos[2] / point_distance_3d(0,0,0, lightpos[0], lightpos[1], lightpos[2])
	];
}
// World-Dependent Lighting
else
{
	lightpos = [8, 32, 48];
}
