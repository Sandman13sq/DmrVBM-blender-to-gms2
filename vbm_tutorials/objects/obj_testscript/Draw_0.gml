...
if (mode == 0)
{
	...
	vertex_submit(vb_starcie_scaled, pr_trianglelist, -1);
	DrawModelDesc("Scaled");
	
	shader_set(shd_simple);
	
	matrix_set(matrix_world, Mat4Translate(x+(sep*i++), y+0, 0));
	vertex_submit(vb_starcie_nocompression, pr_trianglelist, -1);
	DrawModelDesc("No Compression");

	matrix_set(matrix_world, Mat4Translate(x+(sep*i++), y+0, 0));
	vertex_submit(vb_starcie_fullcompression, pr_trianglelist, -1);
	DrawModelDesc("Full Compression");

	matrix_set(matrix_world, Mat4Translate(x+(sep*i++), y+0, 1));
	vertex_submit(vb_starcie_floatcolors, pr_trianglelist, -1);
	DrawModelDesc("Float Colors");

	matrix_set(matrix_world, Mat4Translate(x+(sep*i++), y+0, 1));
	vertex_submit(vb_starcie_edgesonly, pr_linelist, -1);
...
else if (mode == 1)
{
	...
	matrix_set(matrix_world, Mat4Translate(x+(sep*i++), y+0, 0));
	vbm_starcie_uncompressed.Submit();
	DrawModelDesc("Uncompressed");

	matrix_set(matrix_world, Mat4Translate(x+(sep*i++), y+0, 0));
	vbm_starcie_compressed.Submit();
	DrawModelDesc("Compressed");
	
	matrix_set(matrix_world, Mat4Translate(x+(sep*i++), y+0, 0));
	vbm_starcie_vb.Submit();
