...
// Progress Playback
playbackposition = (playbackposition+trk_prm.CalculateTimeStep(game_get_speed(gamespeed_fps))) mod 1;

if (trk_prm.MatrixSpace() == TRK_Space.none) {playbackmode = 1;}
if (trk_prm.TrackSpace() == TRK_Space.none) {playbackmode = 0;}

// Use pre-evaluated matrices
if (playbackmode == 0)
{
	// Use matrices for given frame
	matpose = trk_prm.GetFrameMatricesByPosition(playbackposition);
}
...
else
{
	...
	EvaluateAnimationTracks(
		...
		vbm_kindle_prm.BoneNames(),	// Keys for mapping tracks to indices. 0 for index only
		localpose			// 2D Array of matrices to write local transforms to
		);
	
	CalculateAnimationPose(
		vbm_kindle_prm.BoneParentIndices(),	// Indices of parent bones for each bone
		vbm_kindle_prm.BoneLocalMatrices(),	// Bind pose local matrices for each bone
		vbm_kindle_prm.BoneInverseMatrices(),	// Inverse model matrices for each bone
